...
package starling.text
{
    ...
    import starling.text.hyperlink.HyperlinkQuad;
    import starling.text.hyperlink.HyperlinkTextField;
    ...
    public class TextField extends DisplayObjectContainer
    {
        ...
        // added by Jum -------
        private var mHyperlinkColor:uint = 0x0000ff;
        private var mHyperlinkQuads:Vector.<HyperlinkQuad>;
        private var mHyperlinkTextField:HyperlinkTextField;
        // end of added by Jum -------
        ...
        private function createComposedContents():void
        {
            ...
            */
            ...
            var hyperlinkLocations:Vector.<HyperlinkLocation> = bitmapFont.fillQuadBatch(mQuadBatch,
              width, height, mText, mFontSize, mColor, mHyperlinkColor, hAlign, vAlign, mAutoScale, mKerning);
            // end of added by Jum -------
            ...
            markHyperlinkAreas(hyperlinkLocations);
        }
        
        // added by Jum -------
        private function markHyperlinkAreas(hyperlinkLocations:Vector.<HyperlinkLocation>):void
        {
          if (!mHyperlinkQuads && hyperlinkLocations)
          {
            mHyperlinkQuads = new Vector.<HyperlinkQuad>();
          }
          else if (mHyperlinkQuads)
          {
            // clear all quads
            var length:int = mHyperlinkQuads.length;
            for (var i:int = 0; i < length; i++)
            {
              if (mHyperlinkQuads[i].parent)
              {
                mHyperlinkQuads[i].removeFromParent();
              }
              mHyperlinkQuads[i].dispose();
            }
            mHyperlinkQuads.splice(0, mHyperlinkQuads.length);
          }
          
          if (!hyperlinkLocations) return
          
          var start:HyperlinkLocation;
          var end:HyperlinkLocation;
          length = hyperlinkLocations.length;
          var quad:HyperlinkQuad;
          var lastQuad:HyperlinkQuad;
          var w:Number;
          var h:Number;
          for (i = 0; i < length; i++)
          {
            if (i % 2 == 0)
            {
              start = hyperlinkLocations[i];
            }
            else
            {
              end = hyperlinkLocations[i];
              
              w = end.charLocation.x + (end.charLocation.char.width * end.charLocation.scale) - start.charLocation.x;
              h = Math.max((start.charLocation.char.height * start.charLocation.scale), 
                (end.charLocation.char.height * end.charLocation.scale));
              quad = new HyperlinkQuad(w, h, mHyperlinkColor, start.charLocation.x, Math.min(start.charLocation.y, end.charLocation.y));
              quad.link = start.link;
              if (start.isJoined)
              {
                var fellowQuads:Vector.<HyperlinkQuad> = lastQuad.joinedQuads;
                if (fellowQuads)
                {
                  var fellowQuadsLength:int = fellowQuads.length;
                  for (var j:int=0; j < fellowQuadsLength; j++)
                  {
                    fellowQuads[j].joinQuad(quad);
                    quad.joinQuad(fellowQuads[j]);
                  }
                }
                lastQuad.joinQuad(quad);
                quad.joinQuad(lastQuad);
              }
              mHyperlinkQuads.push(quad);
              lastQuad = quad;
            }
          }
          
          if (!mHyperlinkTextField) mHyperlinkTextField = new HyperlinkTextField(this);
        ...
        public function get hyperlinkColor():uint { return mHyperlinkColor; }
        public function set hyperlinkColor(value:uint):void
        {
            if (mHyperlinkColor != value)
            {
        ...
        // added by Jum -------
        public function get hyperlinkQuads():Vector.<HyperlinkQuad> { return mHyperlinkQuads; }
